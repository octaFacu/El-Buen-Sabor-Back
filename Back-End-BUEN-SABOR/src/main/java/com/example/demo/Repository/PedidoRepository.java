...
public interface PedidoRepository extends GenericRepository<Pedido, Long> {
    ...
    @Query(value = "SELECT pedido_has_producto.pedido_id ,producto.denominacion ,producto.precio_total ,producto.imagen " +
            "FROM pedido_has_producto INNER JOIN producto ON pedido_has_producto.producto_id = producto.id  WHERE pedido_id = :idPedido", nativeQuery = true)
